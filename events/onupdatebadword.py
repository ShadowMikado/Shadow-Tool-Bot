...
class Onupdatebadword(interactions.Extension):
    @interactions.extension_listener(name="on_message_update")
    async def onUpdateBadword(self,oldMessage: interactions.Message, message: interactions.Message):
        banned = banned2
        # banned = ["d   iscord.gg/","dsc.gg/","https://discord.gg/",".gg/","https://dsc.gg/"]
            ...
            if message.content.find(f"{i}") != -1:
                if "/" in message.content:
                    ...
                    webhook = await interactions.Webhook.create(channel_id=int(message.channel_id),
                                                                name=str(message.author.username), client=self.client._http)
